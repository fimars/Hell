...
const NavLabel = ({ level, text, children, location }) => {
  return [
    ...
    children.length ? <div key="post" className="label-post">{ makeNavLabels(children, location) }</div> : null
  ]
}
const makeNavLabels = (nodes, location) => nodes.map((node, index) => <NavLabel key={index} location={location} {...node}/>)
